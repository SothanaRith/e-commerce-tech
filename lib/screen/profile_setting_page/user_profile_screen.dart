...
class _UserProfileScreenState extends State<UserProfileScreen> {
                ...
                actions: <Widget>[
                  buildActionSheetAction('from_gallery'.tr, Colors.blue, () {
                    cropGallery(context).then((image) {
                      if (image != null) {
                        authController.updateUserProfilePicture(context, XFile(
                            image.path));
                      }
                    });
                  }),
  ...
  Widget build(BuildContext context) {
    return GetBuilder<AuthController>(builder: (logic) {
      return Scaffold(
        appBar: customAppBar(
            type: this, title: "user_profile".tr, context: context),
        body: SafeArea(
          child: Skeletonizer(
            enabled: logic.isLoading,
                              ...
                              GestureDetector(
                                ...
                                child: CircleAvatar(
                                  ...
                                  backgroundImage: NetworkImage(
                                      safeImageUrl(logic.userProfile != null ? logic.userProfile ?? 'https://img.freepik.com/free-vector/blue-circle-with-white-user_78370-4707.jpg?semt=ais_hybrid&w=740' : user?.coverImage != null
                                        ? "${user!.coverImage}"
